...
- **[Python 다운로드](https://www.python.org/downloads/)** (3.8 이상)
  - 윈도우 설치 시 **"Add Python to PATH"** 체크박스를 꼭 체크하세요!
...
### 왜 픽셀 폰트인가요? (글씨가 뭉개져 보여요)
Hytale 클라이언트는 **512x512 크기의 폰트 텍스처(Atlas)** 만을 지원하는 것으로 보입니다.
- 한글 약 2,000자를 512x512 공간 안에 모두 넣으려면 글자 크기를 **10px 내외**로 아주 작게 줄여야 합니다.
- 일반적인 고딕 폰트(Vector)는 이 크기에서 뭉개져서 가독성이 떨어집니다.
- 따라서 작은 크기에서도 또렷하게 보이는 **비트맵/픽셀 폰트(갈무리9)** 를 채택했습니다.
- 그럼에도 불구하고 Hytale의 MSDF 렌더링(벡터 전용) 방식 때문에 픽셀이 칼같이 반듯하게 나오지 않고 약간 부드럽게 보일 수 있습니다. 이는 시스템적 한계입니다.
